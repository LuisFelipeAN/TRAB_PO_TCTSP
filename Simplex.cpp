...
int Simplex::equal(double a, double b) {
...
void Simplex::nl(int k) {
    int j;
    for(j=0; j<k; j++) putchar('-');
    putchar('\n');
}

void Simplex::print_tableau(Tableau *tab, const char* mes) {
...
/* Example input file for read_tableau:
     ...
     10    0     1  0  -1
*/
void Simplex::read_tableau(Tableau *tab, const char * filename) {
    int err, i, j;
    FILE * fp;
...
void Simplex::pivot_on(Tableau *tab, int row, int col) {
    int i, j;
    double pivot;
...
int Simplex::find_pivot_column(Tableau *tab) {
    int j, pivot_col = 1;
...
int Simplex::find_pivot_row(Tableau *tab, int pivot_col) {
    int i, pivot_row = 0;
    double min_ratio = -1;
...
void Simplex::add_slack_variables(Tableau *tab) {
    int i, j;
    for(i=1; i<tab->m; i++) {
        for(j=1; j<tab->m; j++)
...
void Simplex::check_b_positive(Tableau *tab) {
    int i;
    for(i=1; i<tab->m; i++)
...
int Simplex::find_basis_variable(Tableau *tab, int col) {
    int i, xi=-1;
    for(i=1; i < tab->m; i++) {
...
void Simplex::print_optimal_vector(Tableau *tab, char *message) {
    int j, xi;
    printf("%s at ", message);
...
void Simplex::simplex(Tableau *tab) {
    int loop=0;
...
tab = { 4, 5, {                                  /// Size of tableau [4 rows x 5 columns ]
